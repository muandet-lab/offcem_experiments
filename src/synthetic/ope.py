...
def run_ope_with_clustering_variants(
    bandit_data: Dict,
    pi_e: np.ndarray,
    estimation_clusters_dict: Dict[str, np.ndarray],
    f_x_a_dict: Dict[str, np.ndarray],
    q_x_a: np.ndarray,
) -> Dict[str, float]:
    """Run OPE with multiple clustering variants for OffCEM estimation.

    Always includes IPS, DR, DM, MIPS (cluster-independent) plus one OffCEM
    variant per entry in estimation_clusters_dict.
    """
    n_actions = bandit_data["n_actions"]
    ...
    base_estimators = [
        ...
        OffCEM(n_actions=n_actions, estimator_name="MIPS"),
    ]
    ope = OffPolicyEvaluation(
        bandit_feedback=bandit_data,
        ope_estimators=base_estimators,
    )
    estimated_policy_values = ope.estimate_policy_values(
        action_dist=pi_e,
        action_embed=bandit_data["action_embed"],
        pi_b=bandit_data["pi_b"],
        p_e_a={"MIPS": bandit_data["p_e_a"]},
    ...
    p_e_a_dict = {}
    reg_model_dict = {}
    for name, est_clusters_3d in estimation_clusters_dict.items():
        est_name = f"OffCEM ({name})"
        offcem_estimators.append(
            OffCEM(n_actions=n_actions, estimator_name=est_name, is_clustering=True)
        )
        p_e_a_dict[est_name] = est_clusters_3d[bandit_data["user_idx"]]
        reg_model_dict[est_name] = f_x_a_dict[name]

    if offcem_estimators:
        ope2 = OffPolicyEvaluation(
            bandit_feedback=bandit_data,
            ope_estimators=offcem_estimators,
        )
        offcem_values = ope2.estimate_policy_values(
            action_dist=pi_e,
            action_embed=bandit_data["action_embed"],
            pi_b=bandit_data["pi_b"],
            p_e_a=p_e_a_dict,
            estimated_rewards_by_reg_model=reg_model_dict,
        )
        estimated_policy_values.update(offcem_values)
